...
let rec print_value = function
  | VInt n -> printf "%d" n
  | VBool b -> printf "%b" b
  | VPair (a,b) -> printf "(" ; print_value a; printf ", "; print_value b; printf ")"
  | VClos _ -> printf "<fun>"
...
let rec eval e env = match e with
  ...
  | Bop(op, e1, e2) ->
     begin match op, eval e1 env, eval e2 env with
       ...
       | Div, VInt n1,  VInt n2  -> begin try VInt (n1 / n2) 
                                    with Division_by_zero -> failwith "Division by zero" end
       | Rem, VInt n1,  VInt n2  -> begin try VInt (n1 mod n2) 
                                    with Division_by_zero -> failwith "Division by zero" end
  ...
  | Match(e, cases) -> 
      let v = eval e env in
          begin match v with
            | VCstr (x, vlist) -> 
                let rec find_case x cases =
                  match cases with
                    | [] -> failwith ("No matching case found for constructor: " ^ x)
                    | (PCstr(pn,plist),e)::cs -> 
                      if pn = x then eval e 
                        (List.fold_left2 (fun env p v -> 
                          match p with
                          | PVar x -> Env.add x v env
                          | PCstr _ -> 
                              failwith "nested constructor patterns are not supported") 
                          env plist vlist)
                      else find_case x cs
                    |_ -> failwith "Invalid case structure"
                in
                find_case x cases
            | _ -> failwith "pattern matching error"
      end
